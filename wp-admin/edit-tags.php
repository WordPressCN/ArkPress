...
switch ( $wp_list_table->current_action() ) {
	...
	default:
		...
		$location = apply_filters( "handle_bulk_actions-{$screen}", $location, $wp_list_table->current_action(), $tags ); // phpcs:ignore ArkPress.NamingConventions.ValidHookName.UseUnderscores
		break;
}
...
if ( 'category' === $taxonomy || 'link_category' === $taxonomy || 'post_tag' === $taxonomy ) {
	...
	if ( 'category' === $taxonomy ) {
		$help .= '<p>' . __( '<a href="https://arkpress.icu/support/article/posts-categories-screen/">Documentation on Categories</a>' ) . '</p>';
	} elseif ( 'link_category' === $taxonomy ) {
		$help .= '<p>' . __( '<a href="https://codex.wordpress.org/Links_Link_Categories_Screen">Documentation on Link Categories</a>' ) . '</p>';
	} else {
		$help .= '<p>' . __( '<a href="https://arkpress.icu/support/article/posts-tags-screen/">Documentation on Tags</a>' ) . '</p>';
	}

	$help .= '<p>' . __( '<a href="https://arkpress.icu/support/">Support</a>' ) . '</p>';
 ...
 */
do_action( "after-{$taxonomy}-table", $taxonomy );  // phpcs:ignore ArkPress.NamingConventions.ValidHookName.UseUnderscores
