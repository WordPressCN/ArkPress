...
class WP_Plugins_List_Table extends WP_List_Table {
	...
	public function no_items() {
		...
		if ( ! empty( $_REQUEST['s'] ) ) {
			...
			if ( ! is_multisite() && current_user_can( 'install_plugins' ) ) {
				echo ' <a href="' . esc_url( admin_url( 'plugin-install.php?tab=search&s=' . urlencode( $s ) ) ) . '">' . __( 'Search for plugins in the ArkPress Plugin Directory.' ) . '</a>';
			}
	...
	public function single_row( $item ) {
		...
		foreach ( $columns as $column_name => $column_display_name ) {
			...
			switch ( $column_name ) {
				...
				case 'description':
					 ...
					 *     @type string   $RequiresWP       The version of ArkPress which the plugin requires.
		...
		if ( ! $compatible_php || ! $compatible_wp ) {
			...
			if ( ! $compatible_php && ! $compatible_wp ) {
				_e( 'This plugin doesn&#8217;t work with your versions of ArkPress and PHP.' );
				...
				_e( 'This plugin doesn&#8217;t work with your version of ArkPress.' );
