<?php
/**
 * ArkPress Plugin Install Administration API
 *
 * @package ArkPress
...
/**
 * Retrieves plugin installer pages from the ArkPress.icu Plugins API.
 *
 ...
 *
 * The second filter, {@see 'plugins_api'}, allows a plugin to override the ArkPress.icu
 ...
 *         @type bool $requires          Whether to return the required ArkPress version. Default true.
 ...
 *         @type bool $compatibility     Whether to return the ArkPress compatibility list. Default true.
...
function plugins_api( $action, $args = array() ) {
	...
	/**
	 * Filters the ArkPress.icu Plugin Installation API arguments.
	 *
	...
	/**
	 * Filters the response for the current ArkPress.icu Plugin Installation API request.
	 *
...
/**
 * Retrieve popular ArkPress plugin tags.
 *
 * @since 2.7.0
 *
 * @param array $args
...
function install_dashboard() {
	...
	<p>
		<?php
		printf(
			/* translators: %s: https://arkpress.icu/plugins/ */
			__( 'Plugins extend and expand the functionality of ArkPress. You may automatically install plugins from the <a href="%s">ArkPress Plugin Directory</a> or upload a plugin in .zip format by clicking the button at the top of this page.' ),
			__( 'https://arkpress.icu/plugins/' )
		);
		?>
	</p>
...
function install_plugins_favorites_form() {
	...
	?>
	<p><?php _e( 'If you have marked plugins as favorites on ArkPress.icu, you can browse them here.' ); ?></p>
	<form method="get">
		<input type="hidden" name="tab" value="favorites" />
		<p>
			<label for="user"><?php _e( 'Your ArkPress.icu username:' ); ?></label>
	...
	switch ( current_filter() ) {
		...
		case 'install_plugins_beta':
			printf(
				...
				'<p>' . __( 'You are using a development version of ArkPress. These feature plugins are also under development. <a href="%s">Learn more</a>.' ) . '</p>',
...
<div id="<?php echo $_tab; ?>-content" class='<?php echo $_with_banner; ?>'>
	<div class="fyi">
		<ul>
			...
			<?php } if ( ! empty( $api->requires ) ) { ?>
				<li>
					<strong><?php _e( 'Requires ArkPress Version:' ); ?></strong>
					<?php
			...
			<?php } if ( ! empty( $api->slug ) && empty( $api->external ) ) { ?>
				<li><a target="_blank" href="<?php echo esc_url( __( 'https://arkpress.icu/plugins/' ) . $api->slug ); ?>/"><?php _e( 'ArkPress.icu Plugin Page &#187;' ); ?></a></li>
		...
		if ( ! empty( $api->ratings ) && array_sum( (array) $api->ratings ) > 0 ) {
			...
			<p class="fyi-description"><?php _e( 'Read all reviews on ArkPress.icu or write your own!' ); ?></p>
			<?php
				...
				<div class="counter-container">
						<span class="counter-label">
							...
							printf(
								...
								"https://arkpress.icu/support/plugin/{$api->slug}/reviews/?filter={$key}",
								$aria_label,
	...
	if ( ! $tested_wp ) {
		...
		_e( '<strong>Warning:</strong> This plugin <strong>has not been tested</strong> with your current version of ArkPress.' );
		echo '</p></div>';
	} elseif ( ! $compatible_wp ) {
		echo '<div class="notice notice-error notice-alt"><p>';
		_e( '<strong>Error:</strong> This plugin <strong>requires a newer version of ArkPress</strong>.' );
		if ( current_user_can( 'update_core' ) ) {
			printf(
				/* translators: %s: URL to ArkPress Updates screen. */
				' ' . __( '<a href="%s" target="_parent">Click here to update ArkPress</a>.' ),
	...
	foreach ( (array) $api->sections as $section_name => $content ) {
		$content = links_add_base_url( $content, 'https://arkpress.icu/plugins/' . $api->slug . '/' );
