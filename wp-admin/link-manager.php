...
if ( $doaction && isset( $_REQUEST['linkcheck'] ) ) {
	...
	if ( 'delete' === $doaction ) {
		...
		$redirect_to = apply_filters( "handle_bulk_actions-{$screen}", $redirect_to, $doaction, $bulklinks ); // phpcs:ignore ArkPress.NamingConventions.ValidHookName.UseUnderscores
	}
	...
	array(
		...
		'content' =>
			'<p>' . sprintf(
				...
				__( 'You can add links here to be displayed on your site, usually using <a href="%s">Widgets</a>. By default, links to several sites in the ArkPress community are included as examples.' ),
				'widgets.php'
			) . '</p>' .
