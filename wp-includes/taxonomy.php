...
function term_exists( $term, $taxonomy = '', $parent = null ) {
	...
	if ( is_int( $term ) ) {
		...
		$where = 't.term_id = %d';
		if ( ! empty( $taxonomy ) ) {
			// phpcs:ignore ArkPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber
...
function wp_unique_term_slug( $slug, $term ) {
	...
	if ( apply_filters( 'wp_unique_term_slug_is_bad_slug', $needs_suffix, $slug, $term ) ) {
		...
		if ( $wpdb->get_var( $query ) ) { // phpcs:ignore ArkPress.DB.PreparedSQL.NotPrepared
			$num = 2;
			do {
				$alt_slug = $slug . "-$num";
				$num++;
...
function _update_post_term_count( $terms, $taxonomy ) {
	...
	foreach ( (array) $terms as $term ) {
		...
		if ( $check_attachments ) {
			// phpcs:ignore ArkPress.DB.PreparedSQLPlaceholders.QuotedDynamicPlaceholderGeneration
