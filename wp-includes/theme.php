 ...
 * ArkPress workflow, then change the former. If you just have the locale in a
...
function get_theme_root_uri( $stylesheet_or_template = '', $theme_root = '' ) {
	 ...
	 * @param string $siteurl                ArkPress web address which is set in General Options.
...
/**
 * Validates the theme requirements for ArkPress version and PHP version.
 *
	...
	if ( ! $compatible_wp && ! $compatible_php ) {
		return new WP_Error(
			...
			sprintf(
				...
				_x( '<strong>Error:</strong> Current ArkPress and PHP versions do not meet minimum requirements for %s.', 'theme' ),
				$theme->display( 'Name' )
			)
		);
		...
		return new WP_Error(
			...
			sprintf(
				/* translators: %s: Theme name. */
				_x( '<strong>Error:</strong> Current ArkPress version does not meet minimum requirements for %s.', 'theme' ),
				$theme->display( 'Name' )
			)
		);
	}
 ...
 * @since 5.9.0 移除了 `core-block-patterns` 支持
 *
...
function current_theme_supports( $feature, ...$args ) {
	 ...
	 */
	return apply_filters( "current_theme_supports-{$feature}", true, $args, $_wp_theme_features[ $feature ] ); // phpcs:ignore ArkPress.NamingConventions.ValidHookName.UseUnderscores
}
 ...
 * ArkPress is loading, especially in the Customizer preview
...
function create_initial_theme_features() {
	...
	register_theme_feature(
		'wp-block-styles',
		array(
			'description'  => __( 'Whether theme opts in to default ArkPress block styles for viewing.' ),
			'show_in_rest' => true,
		)
	);
}
