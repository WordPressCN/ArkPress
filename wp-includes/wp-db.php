<?php
/**
 * ArkPress database access abstraction class
 *
 ...
 * By default, ArkPress uses this class to instantiate the global $wpdb object, providing
 * access to the ArkPress database.
 *
...
class wpdb {
	...
	/**
	 * ArkPress table prefix.
	 *
	 * You can set this to have multiple ArkPress installations in a single database.
	...
	/**
	 * List of ArkPress per-site tables.
	 *
	 * @since 2.5.0
	 *
	 * @see wpdb::tables()
	...
	/**
	 * List of deprecated ArkPress tables.
	 *
	...
	/**
	 * List of ArkPress global tables.
	 *
	 * @since 3.0.0
	 *
	 * @see wpdb::tables()
	...
	/**
	 * ArkPress Comment Metadata table.
	 *
	 * @since 2.9.0
	 *
	 * @var string
	...
	/**
	 * ArkPress Term Relationships table.
	 *
	 * @since 2.3.0
	 *
	 * @var string
	 ...
	 * against the required MySQL version for ArkPress. Normally, a replacement
	...
	/**
	 * Changes the current SQL mode, and ensures its ArkPress compatibility.
	 *
	...
	/**
	 * Sets the table prefix for the ArkPress tables.
	 *
	 * @since 2.5.0
	 *
	...
	/**
	 * Returns an array of ArkPress tables.
	 *
	 * Also allows for the `CUSTOM_USER_TABLE` and `CUSTOM_USER_META_TABLE` to override the ArkPress users
		...
		if ( ! $success ) {
			...
			if ( ! did_action( 'template_redirect' ) ) {
				...
				$message .= '<p>' . sprintf(
					...
					__( 'If you don&#8217;t know how to set up a database you should <strong>contact your host</strong>. If all else fails you may find help at the <a href="%s">ArkPress Support Forums</a>.' ),
	...
	public function print_error( $str = '' ) {
		...
		if ( $caller ) {
			...
			$error_str = sprintf( __( 'ArkPress database error %1$s for query %2$s made by %3$s' ), $str, $this->last_query, $caller );
		} else {
			/* translators: 1: Database error message, 2: SQL query. */
			$error_str = sprintf( __( 'ArkPress database error %1$s for query %2$s' ), $str, $this->last_query );
		}
	...
	public function db_connect( $allow_bail = true ) {
		...
		if ( $this->use_mysqli ) {
			/*
			 * Set the MySQLi error reporting off because ArkPress handles its own.
		...
		if ( ! $this->dbh && $allow_bail ) {
			...
			$message .= '<p>' . sprintf(
				/* translators: %s: Support forums URL. */
				__( 'If you&#8217;re unsure what these terms mean you should probably contact your host. If you still need help you can always visit the <a href="%s">ArkPress Support Forums</a>.' ),
				__( 'https://arkpress.icu/support/forums/' )
			) . "</p>\n";
	...
	public function query( $query ) {
		...
		if ( $this->check_current_query && ! $this->check_ascii( $query ) ) {
			...
			if ( $stripped_query !== $query ) {
				...
				$this->last_error = __( 'ArkPress database error: Could not perform query because it contains invalid data.' );
		...
		if ( $data !== $converted_data ) {
			...
			if ( 1 === count( $problem_fields ) ) {
				$this->last_error = sprintf(
					...
					__( 'ArkPress database error: Processing the value for the following field failed: %s. The supplied value may be too long or contains invalid data.' ),
					reset( $problem_fields )
				);
			} else {
				$this->last_error = sprintf(
					/* translators: %s: Database fields where the error occurred. */
					__( 'ArkPress database error: Processing the values for the following fields failed: %s. The supplied values may be too long or contain invalid data.' ),
	...
	public function check_database_version() {
		...
		if ( version_compare( $this->db_version(), $required_mysql_version, '<' ) ) {
			/* translators: 1: ArkPress version number, 2: Minimum required MySQL version number. */
			return new WP_Error( 'database_version', sprintf( __( '<strong>Error</strong>: ArkPress %1$s requires MySQL %2$s or higher' ), $wp_version, $required_mysql_version ) );
		}
	}
	 ...
	 *
	 * Called when ArkPress is generating the table scheme.
	 *
