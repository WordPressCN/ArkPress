...
/** Make sure that the ArkPress bootstrap has run before continuing. */
 ...
 *
 * @param string   $title    Optional. ArkPress login Page title to display in the `<title>` element.
 *                           Default 'Log In'.
...
function login_header( $title = 'Log In', $message = '', $wp_error = null ) {
	...
	$login_title = sprintf( __( '%1$s &lsaquo; %2$s &#8212; ArkPress' ), $title, $login_title );
	...
	$login_header_url = __( 'https://arkpress.icu/' );
	...
	$login_header_text = empty( $login_header_title ) ? __( 'Powered by ArkPress' ) : $login_header_title;
	...
	case 'confirm_admin_email':
		...
		<form class="admin-email-confirm-form" name="admin-email-confirm-form" action="<?php echo esc_url( site_url( 'wp-login.php?action=confirm_admin_email', 'login_post' ) ); ?>" method="post">
			...
			<p class="admin-email__details">
				...
				/* translators: URL to the ArkPress help section about admin email. */
				$admin_email_help_url = __( 'https://arkpress.icu/support/article/settings-general-screen/#email-address' );
	...
	default:
			...
			if ( headers_sent() ) {
				$user = new WP_Error(
					...
					sprintf(
						...
						__( 'https://arkpress.icu/support/article/cookies/' ),
				...
				$user = new WP_Error(
					...
					sprintf(
						...
						__( '<strong>Error</strong>: Cookies are blocked or not supported by your browser. You must <a href="%s">enable cookies</a> to use ArkPress.' ),
						__( 'https://arkpress.icu/support/article/cookies/#enable-cookies-in-your-browser' )
					)
				);
			}
		}
		...
		if ( $interim_login ) {
			...
			if ( isset( $_GET['loggedout'] ) && $_GET['loggedout'] ) {
				...
				$errors->add( 'updated', __( '<strong>You have successfully updated ArkPress!</strong> Please log back in to see what&#8217;s new.' ), 'message' );
